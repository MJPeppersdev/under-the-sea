...
function setup() {
  var cHeight = constrain(window.innerHeight, 600, 1440);
  var cWidth = constrain(window.innerWidth, 600, 3440);
...
function mousePressed() {
  if ($('.info-options').css('display') === 'none' || mouseX > 350) {
...
function Bloop(l, dna_) {
  ...
  this.maxspeed = map(this.dna.genes[0], 90, 230, 4, 1);
  this.maxforce = map(this.dna.genes[0], 90, 230, 0.40, 0.1);
  this.r = random(12, 30);
  this.acceleration = createVector(0, 0);
  this.velocity = createVector(random(-1, 1), random(-1, 1));
  this.fill = this.dna.genes[0];
  this.sightRadius = map(this.dna.genes[0], 90, 230, 500, 300);
  if (random(1) < 0.25) {
...
function Shark() {
  this.position = createVector(random(-300, -100), random(height));
  this.velocity = createVector();
  this.acceleration = createVector();
